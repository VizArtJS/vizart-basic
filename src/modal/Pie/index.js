...
class Pie extends AbstractBasicCartesianChart {
  ...
  _animate() {
    ...
    const transformedInitial = limitSliceValues(
      initialState,
      this._options,
      this._color
    );
    const transformedFinal = limitSliceValues(
      finalState,
      this._options,
      this._color
    );
    ...
    animateStates(
      ...
      const colorMap = drawHiddenCanvas(that._hiddenContext, res, opt, that);
      /**
      ...
      function mouseMoveHandler() {
        ...
        if (node) {
          ...
          if (isArray(node.data.data)) {
            ...
            n[that._getMetric().accessor] = sum(
